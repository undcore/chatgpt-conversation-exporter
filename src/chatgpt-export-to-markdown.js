(() => {
  ...
  function getMessageText(message) {
    ...
    if (!Array.isArray(parts)) {
      const directText = message?.content?.text;
      return typeof directText === "string" ? normalizeText(directText) : "";
    }
  ...
  function getTitle(conversation, messages) {
    const conversationTitle = typeof conversation?.title === "string" ? normalizeText(conversation.title) : "";
    if (conversationTitle) {
      return conversationTitle.replace(/\s+/g, " ");
    }
  ...
  function makeFilename(title, currentDate) {
  ...
  function buildMarkdown(title, messages, currentDate) {
    const lines = [
      "# " + title,
      "",
      "Exported from ChatGPT on " + currentDate.toISOString().slice(0, 10) + ".",
      ""
    ];
  ...
  function reportError(error) {
    ...
    window.alert(EXPORTER_NAME + ": " + message);
  }

  async function run() {
    const conversationId = getConversationId(window.location.href);
    const conversation = await fetchConversation(conversationId);
    const messages = getActiveBranch(conversation);
    ...
    const markdown = buildMarkdown(title, messages, currentDate);
    const filename = makeFilename(title, currentDate);
    downloadMarkdown(markdown, filename);
    console.info(EXPORTER_NAME + ": downloaded " + filename);
  }
